...
class FMNetBase(BaseModel):
    def __init__(self, opt):
        ...
        if self.with_refine > 0:
            opt['is_train'] = True
        super(FMNetBase, self).__init__(opt)
    
    def data_forward(self, data_x, data_y):
        '''
        This function should be changed for different functions
        '''
    ...
    def feed_data(self, data):
        ...
        feat_x, feat_y = self.data_forward(data_x, data_y)
    ...
    def validation(self, dataloader, tb_logger, update=True):
        ...
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = False
        super(FMNetBase, self).validation(dataloader, tb_logger, update)
        if 'permutation' in self.networks:
            self.networks['permutation'].hard = False
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = True
