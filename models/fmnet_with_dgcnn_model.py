...
class FMNetWithDGCNNModel(BaseModel):
    ...
    def feed_data(self, data):
        ...
        # feature extractor for ocd
        graph_x = to_device(self.create_graph(data_x['verts']))
        graph_y = to_device(self.create_graph(data_y['verts']))

        feat_x, feat_y = self.networks['feature_extractor'](graph_x, graph_y)  # [B, Nx, C]
    ...
    def create_graph(self, vertices: torch.Tensor) -> Data:
        ...
        graph = self.graph_former(graph).to(self.device)
        edges = graph.edge_index
        edge_attr = torch.ones(edges.shape[1], dtype=torch.float32, device=self.device)
    ...
    def validation(self, dataloader, tb_logger, update=True):
        ...
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = False
        super(FMNetWithDGCNNModel, self).validation(dataloader, tb_logger, update)
        if 'permutation' in self.networks:
            self.networks['permutation'].hard = False
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = True
