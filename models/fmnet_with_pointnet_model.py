...
class FMNetWithPointNetModel(BaseModel):
    ...
    def validate_single(self, data, timer):
        ...
        # feature extractor
        feat_x = self.networks['feature_extractor'](verts_x)
        feat_y = self.networks['feature_extractor'](verts_y)
        ...
        if self.use_LBO:
            ...
            evecs_x = data_x['gl_evecs'][:, :, :40].squeeze()
            evecs_y = data_y['gl_evecs'][:, :, :40].squeeze()
    ...
    def validation(self, dataloader, tb_logger, update=True):
        ...
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = False
        super(FMNetWithPointNetModel, self).validation(dataloader, tb_logger, update)
        if 'permutation' in self.networks:
            self.networks['permutation'].hard = False
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = True
